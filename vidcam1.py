...
#логи у файл
#logging.basicConfig(filename='/media/usb/logs/vidcam1.log',format='%(levelname)s[%(asctime)s] %(message)s',level=LOG_LEVEL, datefmt='%m/%d/%Y %I:%M%p')
...
while True:
  ...
  if cam1_state=='1':
    ...
    pro = subprocess.Popen(["/usr/bin/mplayer", "-dumpstream", "http://admin:admin@192.168.1.11/video/mjpg.cgi", "-dumpfile", "/media/usb/cam1/"+filename+".mjpeg"], preexec_fn=os.setsid)
   ...
   #якщо координати коректні, запам'ятати їх як останні коректні
    ...
    os.rename("/media/usb/cam1/"+filename+".mjpeg", '/media/usb/'+rpi_id+'/'+dayname+'/'+filename+".mjpeg")
